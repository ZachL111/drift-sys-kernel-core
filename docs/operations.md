...
`drift-sys-kernel-core` is centered on systems programming work in R. The technical target is: Build an R toolkit that studies kernel behavior through bounded scenario files, with conflict explanations and no credentials or hosted services.
...
The score is deterministic. The current threshold is 162. Risk has a penalty of 6, latency has a penalty of 4, and weight contributes a bonus of 3.
